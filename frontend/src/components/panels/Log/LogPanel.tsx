...
function TurnCard({ turn, index, domainStartMs, domainDurationMs }: TurnCardProps): ReactElement {
  const phases = useMemo(
    () => [
      { label: 'STT', startMs: turn.audio_end_ts, endMs: turn.stt_done_ts, color: PHASE_COLORS[0] },
      { label: 'LLM wait', startMs: turn.stt_done_ts, endMs: turn.llm_first_token_ts, color: PHASE_COLORS[1] },
      { label: 'Streaming', startMs: turn.llm_first_token_ts, endMs: turn.llm_done_ts, color: PHASE_COLORS[2] },
      { label: 'TTS first', startMs: turn.llm_first_token_ts, endMs: turn.tts_first_audio_ts, color: PHASE_COLORS[3] },
      { label: 'TTS total', startMs: turn.tts_first_audio_ts, endMs: turn.tts_done_ts, color: PHASE_COLORS[4] },
    ],
    [turn],
  );
  ...
  return (
    <div className="log-panel">
      <div className="log-tabs">
        <button type="button" className={tabClass('stream')} onClick={() => handleTabClick('stream')}>
          {streamLabel}
        </button>
        <button
          type="button"
