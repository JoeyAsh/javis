...
import type { WindowState, PanelMode, PanelContentRenderProps } from '../primitives/Window';
import { SnapOverlay } from '../primitives/SnapOverlay';
import { SwapOverlay } from '../primitives/SwapOverlay';
import { computeAllSlots, TOP_BAR_HEIGHT } from '../layout/SlotGrid';
...
export interface ManagedWindow {
     ...
     *
     * Replaces the old `content: ReactNode` (breaking change for lib
     * consumers — no external consumers yet beyond the showcase).
     */
...
export interface WindowManagerProps {
     ...
     * instead of its internal mode state. Parent is responsible for toggling.
     */
    modes?: Record<string, PanelMode>;
    /**
     * Called when a window's mode should change. Only fired when `modes` prop
     * is NOT provided (i.e. uncontrolled mode). When `modes` is provided, the
     * parent must listen to `onModesChange` and update accordingly.
     */
    onModesChange?: (next: Record<string, PanelMode>) => void;

    /**
     * Controlled expanded-rect map. When provided, WindowManager uses these
     * values for expanded window positions. Parent must update on drag/resize.
     */
 ...
 *
 * Mode state is uncontrolled by default (internal `useState`). Pass `modes` +
 * `onModesChange` for a fully controlled setup.
 *
 * All assignment state lives in the parent. Internal state covers only:
 * - Current drag (compact slot-swap) or free drag (expanded)
 * - Per-window resizing state
 * - Custom per-window rect overrides from resize (compact)
 * - Modes (compact/expanded) when uncontrolled
 * - Expanded rects when uncontrolled
 */
