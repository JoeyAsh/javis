...
export function detectSnapRegion(
  ...
  if (
    pointerX <= 18 &&
    pointerY > topBar + 40 &&
    pointerY < viewportH - 40
  ) {
    return 'LEFT_HALF';
  }
  if (
    pointerX >= viewportW - 18 &&
    pointerY > topBar + 40 &&
    pointerY < viewportH - 40
  ) {
    return 'RIGHT_HALF';
  ...
  try {
    ...
    for (const id of PANEL_ORDER) {
      ...
      const nextSlot: SlotId = isSlotId(persisted.homeSlotId)
        ...
        : base.homeSlotId;
      const nextMax =
        typeof persisted.maximized === 'boolean'
          ? persisted.maximized
          : base.maximized;
      const rawFloat =
        ...
        typeof persisted.floatingRect.h === 'number'
          ...
          : null;
      const nextZ =
        typeof persisted.zIndex === 'number' ? persisted.zIndex : base.zIndex;
      const nextVisible =
...
export function WindowManagerProvider({
  children,
}: WindowManagerProviderProps): ReactElement {
  ...
  const [settlingIds, setSettlingIds] = useState<ReadonlySet<PanelId>>(
    () => new Set<PanelId>(),
  );
  ...
  const maximize = useCallback(
    (id: PanelId): void => {
      setWindows((prev) => {
        ...
        const maxZ = Object.values(prev).reduce(
